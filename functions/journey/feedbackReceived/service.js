...
async function dispatch({ input = {}, env, journeyRepository, feedbackRepository, log }) {
  ...
  let command;
  try {
    command = validateMailCommand({ automationKey: "journey.feedback_received", templateKey: "journey.feedback_received", templateVersion: 1, journeyEventKey: keys.eventKey, outboxIdempotencyKey: keys.outboxKey, customerReference: customerId, journeyInstanceReference: lookup.row.id, recipient: input.recipient, replyToProfile: { email: "info@maxwebstudio.nl" }, subjectData: { label: clean(input.projectLabel) || "je website" }, templateData: { firstName: clean(input.firstName), projectLabel: clean(input.projectLabel) || "je website", previewVersionLabel: clean(input.previewVersionLabel) || "Website-preview", feedbackCategory: classification.label, feedbackPointCount: classification.count, submittedAt: safeDate(input.submittedAt), percentage: transition.after.percentage, currentPhase: "Feedback verwerken", nextStep: "We verwerken de wijzigingen en laten het weten zodra een bijgewerkte versie klaarstaat", contactName: clean(input.contactName) || "Team Max Webstudio", contactRole: clean(input.contactRole), contactPhone: clean(input.contactPhone) }, actionUrl: "https://maxwebstudio.nl/klantportaal.html#website-review", locale: "nl", metadata: { scenario: "feedback_received_test_customer", previewVersionReference: previewVersionId, feedbackReference: feedbackFingerprint(feedbackId) } }, { customerId, journeyInstanceId: lookup.row.id, environment: "test" }, env);
  } catch (error) {
