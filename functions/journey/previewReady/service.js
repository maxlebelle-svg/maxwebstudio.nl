...
async function dispatch({ input = {}, env, journeyRepository, previewRepository, log }) {
  ...
  try {
    command = validateMailCommand({
      ...
      subjectData: { label: clean(input.businessLabel) || "je website" },
      templateData: { firstName: clean(input.firstName), projectLabel: clean(input.businessLabel) || "je website", previewVersionLabel: clean(input.previewVersionLabel) || "Nieuwe versie", percentage: Number(lookup.row.progress_percent || 70), currentPhase: clean(lookup.row.current_phase) || "Preview en controle", nextStep: "Bekijk de preview en geef je feedback of goedkeuring", contactName: clean(input.contactName) || "Team Max Webstudio", contactRole: clean(input.contactRole), contactPhone: clean(input.contactPhone) },
