...
const TABLE_SELECTS = Object.freeze({
  ...
  invoices: ["invoices", "id,customer_id,status,mollie_payment_status,paid_at,notes,created_at,updated_at"],
  ...
  automationOutbox: ["automation_outbox", "id,event_key,event_type,entity_type,entity_id,effect_type,status,attempt_count,next_attempt_at,processed_at,last_error_code,environment,created_at,updated_at,feedback_reference:payload->>feedbackReference,approval_reference:payload->>approvalReference,payment_reference:payload->>paymentReference,order_reference:payload->>orderReference,invoice_reference:payload->>invoiceReference,website_reference:payload->>websiteReference,project_reference:payload->>projectReference,publication_reference:payload->>publicationReference,publication_source:payload->>publicationSource,live_state:payload->>liveState,url_state:payload->>urlState,dns_state:payload->>dnsState,ssl_state:payload->>sslState,live_hostname_fingerprint:payload->>liveHostnameFingerprint,hostname_category:payload->>hostnameCategory,maintenance_state:payload->>maintenanceState,review_scheduled:payload->>reviewScheduled,provider_category:payload->>providerCategory,payment_environment:payload->>paymentEnvironment,payment_type:payload->>paymentType,commercial_completion_state:payload->>commercialCompletionState,commercial_readiness_state:payload->>commercialReadinessState,paid_component:payload->>paidComponent,remaining_component:payload->>remainingComponent,reminder_cancelled_count:payload->>reminderCancelledCount,ownership_reason:payload->>ownershipReason,journey_type:payload->>journeyType,next_step_type:payload->>nextStepType,payment_state:payload->>paymentState,invoice_state:payload->>invoiceState,customer_action_required:payload->>customerActionRequired,internal_action_required:payload->>internalActionRequired,progress_before:payload->>progressBefore,progress_after:payload->>progressAfter"],
