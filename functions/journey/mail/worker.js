...
async function processItem({ item, context, env, repository, mailSender, log, now }) {
  ...
  try {
    ...
    const sendResult = await mailSender({
      ...
      templateName: command.templateKey === "journey.preview_ready" ? "Journey preview ready v1" : command.templateKey === "journey.feedback_received" ? "Journey feedback received v1" : command.templateKey === "journey.preview_approved" ? "Journey preview approved v1" : command.templateKey === "journey.payment_paid" ? "Journey payment paid v1" : command.templateKey === "journey.website_live" ? "Journey website live v1" : "Journey test status update",
