...
const MODULES = Object.freeze({
  ...
  invoices: {
    table: "invoices",
    select: "id,customer_id,website_id,project_id,source_quote_id,subscription_id,invoice_number,title,total,subtotal,vat,status,due_date,paid_at,pdf_file_path,mollie_payment_id,mollie_checkout_url,mollie_payment_status,created_at,updated_at,notes",
    order: "updated_at.desc.nullslast",
    optional: true,
    salesReadable: true,
    map: mapInvoice,
  ...
  subscriptions: {
    table: "subscriptions",
    select: "id,customer_id,website_id,project_id,plan,total_incl_vat,price_ex_vat,vat_rate,status,billing_cycle,start_date,next_invoice_date,last_invoice_id,created_at,updated_at,metadata",
    order: "updated_at.desc.nullslast",
    optional: true,
    salesReadable: true,
    map: mapSubscription,
...
function mapSubscription(row = {}) {
  return {
    ...
    websiteId: cleanText(row.website_id),
    projectId: cleanText(row.project_id),
    title: cleanText(row.plan),
    plan: cleanText(row.plan),
    packageName: cleanText(row.plan),
    amount: Number(row.total_incl_vat || 0),
    monthlyAmount: Number(row.total_incl_vat || 0),
    currency: "EUR",
