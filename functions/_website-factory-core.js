...
function runQualityCheck({ generatedPackage = {}, journey = {} }) {
  ...
  const checks = [
    ...
    check("static_portfolio_images", "Uitgeklapte dienstbeelden zijn vooraf gevalideerd", portfolioImagesAreStatic && !/portfolioGallery\.innerHTML/.test(script), 10, "technical", true),
...
function normalizeSourceImageUrls(values = []) {
  const blockedHosts = /(^|\.)(?:facebook\.com|facebook\.net|whatsapp\.com|whatsapp\.net|instagram\.com|doubleclick\.net|google-analytics\.com|googletagmanager\.com)$/i;
  const seen = new Set();
  return (Array.isArray(values) ? values : []).map((value) => {
    try {
      const url = new URL(cleanText(value));
      if (url.protocol !== "https:" || blockedHosts.test(url.hostname)) return null;
      if (!/\.(?:avif|gif|jpe?g|png|webp)$/i.test(url.pathname)) return null;
      if (/(?:^|[\/_-])(?:avatar|badge|blank|favicon|icon|logo|pixel|share|spacer|sprite|tracking)(?:[\/_\-.]|$)/i.test(url.pathname)) return null;
...
function renderHtml({ businessName, contactName, email, phone, websiteUrl, siteUrl, industry, region = "", preferSixServiceGrid = false, industryProfile, services, pricingPackages = [], benefits, processSteps, cta, colors, style, title, description, lowInputWarning, packageRules, heroImage, siteAssets, currentWebsite = {}, googleReviews = [], googleRating = "", googleRatingTotal = "", googleMapsUrl = "" }) {
  ...
  const portfolioGallerySets = portfolioServices.map((service, serviceIndex) => {
    ...
    return `<div class="portfolio-gallery" data-portfolio-service="${escapeHtml(service)}" hidden>
          ${galleryServices.map((galleryService, imageIndex) => `<article class="portfolio-item"><img src="${escapeHtml(serviceAssetPath(siteAssets, galleryService, heroAsset))}" alt="${escapeHtml(`${service} voorbeeld ${imageIndex + 1}`)}" loading="lazy" /><strong>${escapeHtml(galleryService)}</strong></article>`).join("")}
        </div>`;
  }).join("");
  ...
  const sourceImages = normalizeSourceImageUrls(currentWebsite.imageUrls);
  ...
  <body style="--brand:${escapeHtml(colors.brand)};--accent:${escapeHtml(colors.accent)};--ink:${escapeHtml(colors.ink)};--soft:${escapeHtml(colors.soft)};--dark:${escapeHtml(colors.dark || colors.ink)}">
    ...
    <main id="top">
      ...
      <section class="section-band services-section" id="diensten" data-mws-section-id="home.services" data-mws-section-type="services" data-mws-section-label="Diensten">
        ...
        <div class="service-grid${displayedServices.length === 6 ? " services-count-6" : ""}" data-mws-field="items">${serviceTiles}</div>
      </section>
      ${pricingSection}
      ...
      <section class="portfolio-panel section-band" id="portfolio" hidden>
        ...
        <div id="portfolioGalleries">${portfolioGallerySets}</div>
      </section>
...
function renderCss() {
  ...
  const sixServiceGridCss = `.services-count-6{grid-template-columns:repeat(3,minmax(0,1fr))}@media(max-width:1100px){.services-count-6{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:760px){.services-count-6{grid-template-columns:minmax(0,1fr)}}`;
  const socialCss = `.nav-contact{display:flex;align-items:center;justify-content:flex-end;gap:12px}.social-links{display:flex;align-items:center;flex-wrap:wrap;gap:7px}.social-links a{display:inline-flex;align-items:center;justify-content:center;min-height:30px;padding:5px 9px;border:1px solid rgba(255,255,255,.28);border-radius:999px;color:#fff;text-decoration:none;font-size:12px;font-weight:900}.footer-socials{margin-top:12px}.footer-socials a{color:#fff}@media(max-width:1100px){.nav-contact .social-links{display:none}}@media(max-width:760px){.nav-contact{display:none}}`;
  ...
  return `${css}${sixServiceGridCss}${socialCss}${tabletSafetyCss}${responsiveSafetyCss}`;
}
...
const portfolioGalleries = Array.from(document.querySelectorAll("[data-portfolio-service]"));
document.querySelectorAll("[data-service]").forEach((card) => {
  card.addEventListener("click", (event) => {
    event.preventDefault();
    const service = card.getAttribute("data-service");
    const data = portfolioData[service];
    const activeGallery = portfolioGalleries.find((gallery) => gallery.getAttribute("data-portfolio-service") === service);
    if (!data || !portfolioPanel || !activeGallery) return;
    portfolioTitle.textContent = data.title;
    portfolioCopy.textContent = data.copy;
    portfolioGalleries.forEach((gallery) => { gallery.hidden = gallery !== activeGallery; });
