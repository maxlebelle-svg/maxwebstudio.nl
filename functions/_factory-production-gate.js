...
const FOOD_CHECKS = Object.freeze([
  definition("restaurant_tenant", "food_foundation", "Restauranttenant", "food_demo_bundle", ["factory_type", "blueprint_key", "relationship_type", "relationship_id"], 24),
  definition("menu_opening_hours", "food_foundation", "Menu en openingstijden", "food_runtime_catalog", ["configuration.foodRuntime"], 4),
  definition("manager_tenant_isolation", "access", "Manageraccount en tenantisolatie", "food_access_context", ["configuration.foodRuntime"], 4),
  definition("order_route", "experience", "Bestelroute", "food_demo_bundle", ["relationship_type", "relationship_id", "blueprint_key"], 1),
  definition("dashboard_view", "experience", "Dashboardweergave", "food_demo_bundle", ["relationship_type", "relationship_id", "blueprint_key"], 1),
  definition("mobile_validation", "experience", "Mobiele controle", "food_storefront_probe", ["relationship_type", "relationship_id", "blueprint_version"], 24),
  definition("domain_mapping", "domain", "Domeinkoppeling", "domain_center", ["relationship_type", "relationship_id"], 1),
  definition("dns_verified", "domain", "DNS gecontroleerd", "domain_center", ["relationship_type", "relationship_id"], 1),
  definition("ssl_active", "domain", "SSL actief", "domain_center", ["relationship_type", "relationship_id"], 1),
  definition("business_email_preserved", "domain", "Zakelijke e-mail behouden", "domain_center", ["relationship_type", "relationship_id"], 24),
  definition("mollie_connected", "commerce", "Mollie verbonden", "commerce", ["relationship_type", "relationship_id"], 1),
  definition("legal_set", "legal", "Juridische set", "legal_registry", ["relationship_type", "relationship_id"], 24),
  ...
  definition("environment_mode", "release", "Demo- of productiemodus", "factory_context", ["factory_type", "blueprint_key", "configuration.environmentMode"], 24),
]);
...
function definitionsFor(project = {}) {
  const rows = project.factory_type === "food" ? FOOD_CHECKS : GENERIC_CHECKS;
  return rows.map((item) => ({ ...item, dependencies: [...item.dependencies], allowedAttestorRoles: [...item.allowedAttestorRoles] }));
}
...
module.exports = { FOOD_CHECKS, GENERIC_CHECKS, RESULT_STATUSES, definitionFor, definitionsFor, fingerprintFor, normalizeSupplierResult, summarizeGate };
