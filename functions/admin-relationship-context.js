...
exports.handler = async (event) => {
  ...
  try {
    ...
    if ((entityType === "lead") === (entityType === "customer") || (leadId && customerId)) return failure(409, "CONTEXT_MISMATCH", "Kies één geldige lead of klant.");
    const id = entityType === "lead" ? leadId : customerId;
    if (!UUID.test(id)) return failure(400, "INVALID_ID", "Kies een geldige relatie.");
    ...
    return failure(error.status || 500, error.status ? "QUERY_FAILED" : "INTERNAL_ERROR", error.status ? error.message : "De relatie kon niet veilig worden geladen.");
  }
};
...
exports._test = { canAccess, isUnavailable, mapLead, mapCustomer };
