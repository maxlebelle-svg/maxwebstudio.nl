exports.handler = async (event) => {
  ...
  try {
    const session = await verifyTokenHash(supabaseUrl, anonKey, tokenHash);
...
async function verifyTokenHash(supabaseUrl, anonKey, tokenHash) {
  ...
  if (!response.ok || !session?.access_token) {
    ...
    error.code = cleanText(payload.error_code || payload.code || payload.error) || "SUPABASE_RECOVERY_VERIFY_FAILED";
    throw error;
  }
  return session;
}
