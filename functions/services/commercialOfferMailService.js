...
function buildCommercialOfferMail(input = {}) {
  ...
  const testBanner = staging
    ? `<tr><td style="padding:12px 28px;background:#fff1bd;color:#5b4200;font-weight:900;text-align:center;">STAGINGTEST — niet naar een echte klant verzenden</td></tr>`
    : test ? `<tr><td style="padding:12px 28px;background:#fff1bd;color:#5b4200;font-weight:900;text-align:center;">TESTMAIL — niet naar de klant verzonden</td></tr>` : "";
  const validityMarkup = `<p style="margin:16px 0 0;color:#526170;font-size:14px;font-weight:700;">Geldig tot en met ${escape(formatValidityDate(validUntil))}</p>`;
  const cta = validityMarkup + (test || preview
  ...
  const text = [
    staging ? "STAGINGTEST — niet naar een echte klant verzenden" : test ? "TESTMAIL — niet naar de klant verzonden" : "",
    ...
    `Geldig tot en met: ${formatValidityDate(validUntil)}`,
