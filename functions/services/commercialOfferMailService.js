...
const EMAIL_BACKGROUNDS = Object.freeze({
  ...
  header: `${SITE_URL}/assets/email/mws-email-bg-header-061523-v2.png`,
  panel: `${SITE_URL}/assets/email/mws-email-bg-panel-102a3d-v2.png`,
  sign: `${SITE_URL}/assets/email/mws-email-bg-sign-08283b-v2.png`,
});
...
function buildCommercialOfferMail(input = {}) {
  ...
  const customerAction = test || preview
    ...
    : `<a class="mws-cta" href="${escape(actionUrl)}" style="display:block;margin-top:22px;padding:15px 18px;border-radius:12px;background:#24d3ee;color:#03111f;text-decoration:none;text-align:center;font-weight:900;">${definitiveOffer ? "Bekijk offerte en onderteken" : "Ja, ik wil verder met deze demo"}</a>`;
  const primarySigningAction = definitiveOffer
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;"><tr><td bgcolor="#08283b" background="${EMAIL_BACKGROUNDS.sign}" style="padding:18px;border:1px solid #17627b;border-radius:16px;background:#08283b url('${EMAIL_BACKGROUNDS.sign}') repeat;"><div style="margin:0 0 5px;color:#24d3ee;font-size:11px;font-weight:900;letter-spacing:.14em;text-transform:uppercase;">Volgende stap</div><div style="color:#ffffff;font-size:16px;line-height:1.55;font-weight:800;">Controleer de offerte en start de beveiligde ondertekening via Signhost.</div>${customerAction}</td></tr></table>`
    : "";
  ...
  const demoLinks = foodDemo
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td bgcolor="#155eef" style="border-radius:12px;background:#155eef;"><a class="mws-cta" href="${escape(storefrontUrl)}" style="display:block;padding:14px 16px;border-radius:12px;background:#155eef;color:#ffffff;text-decoration:none;text-align:center;font-weight:900;">Bekijk de bestelpagina</a></td></tr><tr><td height="10" style="height:10px;line-height:10px;font-size:0;">&nbsp;</td></tr><tr><td bgcolor="#0a3148" style="border:1px solid #17627b;border-radius:12px;background:#0a3148;"><a class="mws-cta" href="${escape(restaurantPortalUrl)}" style="display:block;padding:14px 16px;border-radius:12px;background:#0a3148;color:#a5f3fc;text-decoration:none;text-align:center;font-weight:900;">Open het restaurantportaal</a></td></tr></table>`
    : `<table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td bgcolor="#155eef" style="border-radius:12px;background:#155eef;"><a class="mws-cta" href="${escape(desktopUrl)}" style="display:block;padding:14px 16px;border-radius:12px;background:#155eef;color:#ffffff;text-decoration:none;text-align:center;font-weight:900;">Demo op computer bekijken</a></td></tr><tr><td height="10" style="height:10px;line-height:10px;font-size:0;">&nbsp;</td></tr><tr><td bgcolor="#0a3148" style="border:1px solid #17627b;border-radius:12px;background:#0a3148;"><a class="mws-cta" href="${escape(mobileUrl)}" style="display:block;padding:14px 16px;border-radius:12px;background:#0a3148;color:#a5f3fc;text-decoration:none;text-align:center;font-weight:900;">Mobiele demo openen</a></td></tr></table>`;
...
<body bgcolor="#030b14" style="margin:0;background:#030b14 url('${EMAIL_BACKGROUNDS.outer}') repeat;color:#ffffff;font-family:Inter,Arial,sans-serif;"><div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;">Je persoonlijke demo en voorstel van Max Webstudio staan klaar.</div>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="mws-shell" bgcolor="#030b14" background="${EMAIL_BACKGROUNDS.outer}" style="background:#030b14 url('${EMAIL_BACKGROUNDS.outer}') repeat;padding:30px 14px;"><tr><td align="center"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="mws-card" bgcolor="#071b2c" background="${EMAIL_BACKGROUNDS.card}" style="max-width:680px;background:#071b2c url('${EMAIL_BACKGROUNDS.card}') repeat;border:1px solid #163d58;border-top:4px solid #24d3ee;border-radius:20px;overflow:hidden;">
${bannerRow}<tr><td class="mws-pad" bgcolor="#061523" background="${EMAIL_BACKGROUNDS.header}" style="padding:27px 30px 20px;background:#061523 url('${EMAIL_BACKGROUNDS.header}') repeat;border-bottom:1px solid #163d58;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td width="52" bgcolor="#030b14" background="${EMAIL_BACKGROUNDS.outer}" style="width:52px;height:52px;border-radius:12px;background:#030b14 url('${EMAIL_BACKGROUNDS.outer}') repeat;text-align:center;"><img src="${LOGO_URL}" width="46" height="46" alt="Max Webstudio" style="display:block;margin:3px auto;border:0;width:46px;height:46px;"></td><td style="padding-left:14px;"><div style="font-size:20px;color:#ffffff;font-weight:900;">Max Webstudio</div><div style="margin-top:3px;font-size:10px;color:#24d3ee;font-weight:900;letter-spacing:.18em;text-transform:uppercase;">BUILD BETTER ONLINE</div></td><td align="right" style="color:#8eefff;font-size:11px;font-weight:900;letter-spacing:.1em;text-transform:uppercase;">${definitiveOffer ? "Definitieve offerte" : "Persoonlijk voorstel"}</td></tr></table></td></tr>
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="mws-demo-grid" bgcolor="#102a3d" background="${EMAIL_BACKGROUNDS.panel}" style="background:#102a3d url('${EMAIL_BACKGROUNDS.panel}') repeat;border-radius:16px;"><tr><td style="padding:18px;">${demoLinks}</td><td width="150" class="mws-qr" style="padding:18px 18px 18px 0;text-align:right;"><img src="${escape(qrCodeUrl)}" width="120" height="120" alt="QR-code naar de mobiele demo" style="display:inline-block;background:#ffffff;padding:7px;border-radius:14px;width:120px;height:120px;"></td></tr></table>
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="#102a3d" background="${EMAIL_BACKGROUNDS.panel}" style="margin-top:18px;background:#102a3d url('${EMAIL_BACKGROUNDS.panel}') repeat;border-radius:14px;">${discountHtmlRows}<tr><td style="padding:15px;color:#91a6bc;">Eenmalig${discountPercentage > 0 ? " na korting" : ""} excl. btw</td><td align="right" style="padding:15px;color:#ffffff;font-weight:900;">${escape(money(snapshot.oneTimeExVatCents))}</td></tr><tr><td style="padding:0 15px 15px;color:#91a6bc;">Per maand excl. btw</td><td align="right" style="padding:0 15px 15px;color:#ffffff;font-weight:900;">${escape(money(snapshot.recurringExVatCents))}</td></tr><tr><td style="padding:0 15px 15px;color:#91a6bc;">Vaste aanbetaling excl. btw</td><td align="right" style="padding:0 15px 15px;color:#ffffff;font-weight:900;">${escape(money(Math.min(Number(snapshot.fixedDepositExVatCents || 0), Number(snapshot.oneTimeExVatCents || 0))))}</td></tr></table>
