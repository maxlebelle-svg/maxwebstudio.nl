...
function buildCommercialOfferMail(input = {}) {
  ...
  const customerAction = test || preview
    ? `<div class="mws-panel" style="margin-top:22px;padding:14px;border:1px dashed #47718f;border-radius:14px;background-color:#102a3d;background-image:linear-gradient(#102a3d,#102a3d);color:#91a6bc;text-align:center;font-weight:800;">${test ? "TEST" : "VOORBEELD"}: de ${definitiveOffer ? "Signhost-ondertekenknop" : "interesseknop"} wordt pas in de definitieve klantmail geactiveerd</div>`
  ...
  const primarySigningAction = definitiveOffer
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;"><tr><td class="mws-sign-panel" bgcolor="#08283b" style="padding:18px;border:1px solid #17627b;border-radius:16px;background-color:#08283b;background-image:linear-gradient(#08283b,#08283b);"><div style="margin:0 0 5px;color:#24d3ee;font-size:11px;font-weight:900;letter-spacing:.14em;text-transform:uppercase;">Volgende stap</div><div style="color:#ffffff;font-size:16px;line-height:1.55;font-weight:800;">Controleer de offerte en start de beveiligde ondertekening via Signhost.</div>${customerAction}</td></tr></table>`
    : "";
  const bannerRow = banner ? `<tr><td class="mws-test" bgcolor="#061523" style="padding:14px 28px;background-color:#061523;background-image:linear-gradient(#061523,#061523);border-top:4px solid #fbbf24;color:#ffe29a;-webkit-text-fill-color:#ffe29a;font-weight:900;text-align:center;letter-spacing:.03em;">${escape(banner)}</td></tr>` : "";
...
<body class="mws-body" bgcolor="#030b14" style="margin:0;background-color:#030b14;background-image:linear-gradient(#030b14,#030b14);color:#ffffff;font-family:Inter,Arial,sans-serif;"><div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;">Je persoonlijke demo en voorstel van Max Webstudio staan klaar.</div>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="mws-shell" bgcolor="#030b14" style="background-color:#030b14;background-image:linear-gradient(#030b14,#030b14);padding:30px 14px;"><tr><td align="center"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="mws-card" bgcolor="#071b2c" style="max-width:680px;background-color:#071b2c;background-image:linear-gradient(#071b2c,#071b2c);border:1px solid #163d58;border-top:4px solid #24d3ee;border-radius:20px;overflow:hidden;">
${bannerRow}<tr><td class="mws-pad mws-header" bgcolor="#061523" style="padding:27px 30px 20px;background-color:#061523;background-image:linear-gradient(#061523,#061523);border-bottom:1px solid #163d58;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td width="52" bgcolor="#030b14" style="width:52px;height:52px;border-radius:12px;background-color:#030b14;background-image:linear-gradient(#030b14,#030b14);text-align:center;"><img src="${LOGO_URL}" width="46" height="46" alt="Max Webstudio" style="display:block;margin:3px auto;border:0;width:46px;height:46px;"></td><td style="padding-left:14px;"><div style="font-size:20px;color:#ffffff;font-weight:900;">Max Webstudio</div><div style="margin-top:3px;font-size:10px;color:#24d3ee;font-weight:900;letter-spacing:.18em;text-transform:uppercase;">BUILD BETTER ONLINE</div></td><td align="right" style="color:#8eefff;font-size:11px;font-weight:900;letter-spacing:.1em;text-transform:uppercase;">${definitiveOffer ? "Definitieve offerte" : "Persoonlijk voorstel"}</td></tr></table></td></tr>
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="mws-demo-grid mws-panel" bgcolor="#102a3d" style="background-color:#102a3d;background-image:linear-gradient(#102a3d,#102a3d);border-radius:16px;"><tr><td style="padding:18px;">${demoLinks}</td><td width="150" class="mws-qr" style="padding:18px 18px 18px 0;text-align:right;"><img src="${escape(qrCodeUrl)}" width="120" height="120" alt="QR-code naar de mobiele demo" style="display:inline-block;background:#ffffff;padding:7px;border-radius:14px;width:120px;height:120px;"></td></tr></table>
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="mws-panel" bgcolor="#102a3d" style="margin-top:18px;background-color:#102a3d;background-image:linear-gradient(#102a3d,#102a3d);border-radius:14px;">${discountHtmlRows}<tr><td style="padding:15px;color:#91a6bc;">Eenmalig${discountPercentage > 0 ? " na korting" : ""} excl. btw</td><td align="right" style="padding:15px;color:#ffffff;font-weight:900;">${escape(money(snapshot.oneTimeExVatCents))}</td></tr><tr><td style="padding:0 15px 15px;color:#91a6bc;">Per maand excl. btw</td><td align="right" style="padding:0 15px 15px;color:#ffffff;font-weight:900;">${escape(money(snapshot.recurringExVatCents))}</td></tr><tr><td style="padding:0 15px 15px;color:#91a6bc;">Vaste aanbetaling excl. btw</td><td align="right" style="padding:0 15px 15px;color:#ffffff;font-weight:900;">${escape(money(Math.min(Number(snapshot.fixedDepositExVatCents || 0), Number(snapshot.oneTimeExVatCents || 0))))}</td></tr></table>
