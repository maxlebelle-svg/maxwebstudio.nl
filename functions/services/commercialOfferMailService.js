...
function buildCommercialOfferMail(input = {}) {
  ...
  const customerAction = test || preview
    ? `<div style="margin-top:22px;padding:14px;border:1px dashed #47718f;border-radius:14px;color:#91a6bc;text-align:center;font-weight:800;">${test ? "TEST" : "VOORBEELD"}: de interesseknop wordt pas in de definitieve klantmail geactiveerd</div>`
...
<tr><td class="mws-pad" bgcolor="#102a3d" style="padding:24px 30px 26px;background:#102a3d;border-top:1px solid #29445a;"><strong style="color:#ffffff;font-size:16px;">Max Webstudio</strong><p style="margin:7px 0 14px;color:#91a6bc;font-size:13px;line-height:1.6;">Professionele websites voor ondernemers die snel vertrouwen en aanvragen willen.</p><a href="mailto:${supportEmail}" style="color:#7dd3fc;text-decoration:underline;">${supportEmail}</a><span style="color:#587088;"> · </span><a href="https://wa.me/31851302326" style="color:#7dd3fc;text-decoration:underline;">WhatsApp</a><span style="color:#587088;"> · </span><a href="${SITE_URL}" style="color:#7dd3fc;text-decoration:underline;">maxwebstudio.nl</a></td></tr>
  ...
  const text = [banner, `Hoi ${recipientName},`, "", `Je demo en persoonlijke voorstel voor ${companyName} staan klaar.`, "", ...demoTextLinks, "", "Jouw voorstel:", ...textLines, "", ...discountTextLines, `Eenmalig${discountPercentage > 0 ? " na korting" : ""} excl. btw: ${money(snapshot.oneTimeExVatCents)}`, `Per maand excl. btw: ${money(snapshot.recurringExVatCents)}`, `Vaste aanbetaling excl. btw: ${money(Math.min(Number(snapshot.fixedDepositExVatCents || 0), Number(snapshot.oneTimeExVatCents || 0)))}`, `Geldig tot en met: ${formatValidityDate(validUntil)}`, "", test ? "TEST: de interesseknop is niet actief." : preview ? "VOORBEELD: de interesseknop is nog niet actief." : `Ja, ik wil verder met deze demo: ${interestUrl}`, "", disclaimer, "", `Vragen? ${supportEmail}`].filter((line, index, rows) => !(line === "" && rows[index - 1] === "")).join("\n");
  return { subject, html, text, desktopUrl, mobileUrl, storefrontUrl, restaurantPortalUrl, qrCodeUrl, interestUrl, disclaimer, validUntil, test, preview, staging };
}
