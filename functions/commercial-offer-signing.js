...
exports.handler = async (event) => {
  ...
  let failureContext=null; let createdTransactionId="";
  try {
    ...
    const tx=await insert(context,"commercial_offer_signing_transactions",{
      ...
      signer_name:signerName,signer_role:signerRole,signer_email_sha256:sha256(details.relationship.email.toLowerCase()),
    ...
    const providerTx=await createCommercialOfferTransaction(provider,{signerEmail:details.relationship.email,signerName,signerPhone:normalizedPhone,companyName:details.relationship.companyName,reference:pdf.reference});
    ...
    await uploadFileMetadata(provider,providerId,fileId,buildCommercialOfferMetadata(providerTx,{signerEmail:details.relationship.email,signaturePage:pdf.signaturePage,reference:pdf.reference}));
    ...
    await patch(context,"commercial_offer_signing_access_tokens",access.id,{started_at:new Date().toISOString()});
    return json(201,{success:true,duplicate:false,signing:{...publicSigning(updated),redirectUrl:signUrl||null,emailSent:true}});
  } catch(error){
    if(failureContext&&createdTransactionId)await patch(failureContext,"commercial_offer_signing_transactions",createdTransactionId,{status:"failed",failure_code:clean(error.code||"provider_error").toLowerCase().replace(/[^a-z0-9_-]+/g,"_").slice(0,120),updated_at:new Date().toISOString()}).catch(()=>{});
...
async function loadDetails(context,access){
  ...
  const relationship={companyName:clean(row?.company_name||row?.company||row?.name),contactName:clean(row?.contact_name||row?.name),email:clean(row?.email).toLowerCase()};
  if(!relationship.email)throw coded("SIGNER_EMAIL_MISSING",409,"Bij deze relatie ontbreekt een geldig e-mailadres.");
  ...
  return{access,offer,version,relationship,documents,transaction:transactions[0]||null};
}
