...
async function readAdminJourney({ event, supabaseUrl, serviceRoleKey, admin, observePostRead }) {
  ...
  const [events, factoryHistory, projectWorkspace, publicPreviewPublication] = selected
    ? await Promise.all([
      observeReadPhase("events", () => readEvents({ supabaseUrl, serviceRoleKey, journeyId: selected.id })),
      readFactoryHistorySafe({ supabaseUrl, serviceRoleKey, admin, journeyId: selected.id, observeReadPhase }),
      observeReadPhase("project_workspace", () => readProjectWorkspace({ supabaseUrl, serviceRoleKey, admin }, { demoJourneyId: selected.id })),
  ...
  const responseFactoryHistory = projectBuildHistoryMetadata(safeFactoryHistory);
  const response = jsonResponse(200, { success: true, journey: responseSelected, demoJourney: responseSelected, records: responseJourneys, events, templates: emailTemplates(), buildHistory: responseFactoryHistory, buildStatus: responseFactoryHistory.latestJob || null, projectWorkspace, publicPreviewPublication });
...
async function readFactoryHistorySafe({ supabaseUrl, serviceRoleKey, admin, journeyId, observeReadPhase }) {
  try {
    return normalizeFactoryHistorySources(await getBuildHistory({ supabaseUrl, serviceRoleKey, admin, observeReadPhase }, { demoJourneyId: journeyId }));
  } catch (error) {
