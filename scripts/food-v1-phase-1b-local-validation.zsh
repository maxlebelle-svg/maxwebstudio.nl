...
food_validation_root=$(mktemp -d /private/tmp/food-v1-phase-1b.XXXXXXXX)
food_cluster_dir=$food_validation_root/postgres
food_socket_dir=$food_validation_root/socket
food_database=food_v1_phase_1b
...
food_cleanup() {
  ...
  if [[ $food_validation_root == /private/tmp/food-v1-phase-1b.* ]]; then
...
for food_migration in \
  ...
  supabase/migrations/20260728162000_food_v1_application_api_support.sql; do
  $food_pg_bin/psql -h $food_socket_dir -d $food_database -v ON_ERROR_STOP=1 \
    -f $food_repo_root/$food_migration >/dev/null
done

food_phase_1a_output=$($food_pg_bin/psql -h $food_socket_dir -d $food_database -v ON_ERROR_STOP=1 -AtX \
  -f $food_repo_root/tests/fixtures/food-v1-phase-1a-functional.sql)
[[ $food_phase_1a_output == *PASS_FOOD_V1_PHASE_1A_FUNCTIONAL* ]] || { print -u2 "Phase 1A fixture did not pass"; exit 1; }
food_phase_1b_output=$($food_pg_bin/psql -h $food_socket_dir -d $food_database -v ON_ERROR_STOP=1 -AtX \
  -f $food_repo_root/tests/fixtures/food-v1-phase-1b-functional.sql)
[[ $food_phase_1b_output == *PASS_FOOD_V1_PHASE_1B_FUNCTIONAL* ]] || { print -u2 "Phase 1B fixture did not pass"; exit 1; }
...
print -r -- "status=PASS_FOOD_V1_PHASE_1B_LOCAL_VALIDATION"
print -r -- "database=isolated Unix-socket-only PostgreSQL cluster"
print -r -- "production_contact=false"
print -r -- "functional=confirmation-isolation,durable-rate-limit,service-only-rpcs,phase-1a-regression"
