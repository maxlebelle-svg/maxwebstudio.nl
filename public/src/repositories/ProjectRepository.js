...
export function mapSupabaseProjectToLocal(row = {}) {
  return normalizeProject({
    ...
    deadline: row.deadline || row.expected_delivery_date,
    expectedDeliveryDate: row.expected_delivery_date || row.deadline,
    ...
    checklist: row.checklist || row.metadata?.checklist || [],
    tasks: row.tasks || row.metadata?.tasks || [],
