...
export function normalizeLeadFinderLead(lead = {}) {
  ...
  return {
    ...
    ownerEmail: sanitizeString(lead.ownerEmail || lead.owner_email || lead.medewerkerEmail || lead.medewerker_email || lead.employeeEmail || lead.employee_email || lead.assignedToEmail || lead.assigned_to_email || lead.userEmail || lead.user_email || metadata.ownerEmail || metadata.owner_email || metadata.medewerkerEmail || metadata.medewerker_email || metadata.employeeEmail || metadata.employee_email || metadata.assignedToEmail || metadata.assigned_to_email || metadata.userEmail || metadata.user_email),
    ownerName: sanitizeString(lead.ownerName || lead.owner_name || lead.medewerker || lead.employee || lead.assignedToName || lead.assigned_to_name || lead.userName || lead.user_name || metadata.ownerName || metadata.owner_name || metadata.medewerker || metadata.employee || metadata.assignedToName || metadata.assigned_to_name || metadata.userName || metadata.user_name),
