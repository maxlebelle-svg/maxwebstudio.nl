...
export const LEADFINDER_LIFECYCLE_STATUSES = Object.freeze([
  ...
  { value: "call_scheduled", label: "Belmoment gepland" },
  { value: "contact_attempted", label: "Belpoging gedaan" },
  { value: "contacted", label: "Contact gehad" },
  { value: "follow_up", label: "Opvolgen" },
  { value: "appointment_scheduled", label: "Afspraak gepland" },
  { value: "demo_requested", label: "Demo aangevraagd" },
  { value: "demo_building", label: "Demo in productie" },
...
export function normalizeLeadFinderLead(lead = {}) {
  ...
  return {
    ...
    lastContactedBy: sanitizeString(lead.lastContactedBy || lead.last_contacted_by || metadata.lastContactedBy || metadata.last_contacted_by),
    lastCallOutcome: sanitizeString(lead.lastCallOutcome || lead.last_call_outcome || metadata.lastCallOutcome || metadata.last_call_outcome),
    nextActionType: sanitizeString(lead.nextActionType || lead.next_action_type || metadata.nextActionType || metadata.next_action_type),
    nextActionAt: sanitizeString(lead.nextActionAt || lead.next_action_at || metadata.nextActionAt || metadata.next_action_at),
    nextActionNote: sanitizeString(lead.nextActionNote || lead.next_action_note || metadata.nextActionNote || metadata.next_action_note),
    nextActionAssignedUserId: sanitizeString(lead.nextActionAssignedUserId || lead.next_action_assigned_user_id || metadata.nextActionAssignedUserId || metadata.next_action_assigned_user_id),
    nextActionCreatedAutomatically: Boolean(lead.nextActionCreatedAutomatically ?? lead.next_action_created_automatically ?? metadata.nextActionCreatedAutomatically ?? metadata.next_action_created_automatically),
    appointmentAt: sanitizeString(lead.appointmentAt || lead.appointment_at || metadata.appointmentAt || metadata.appointment_at),
    appointmentType: sanitizeString(lead.appointmentType || lead.appointment_type || metadata.appointmentType || metadata.appointment_type),
    appointmentLocation: sanitizeString(lead.appointmentLocation || lead.appointment_location || metadata.appointmentLocation || metadata.appointment_location),
    wonAt: sanitizeString(lead.wonAt || lead.won_at || metadata.wonAt || metadata.won_at),
    wonBy: sanitizeString(lead.wonBy || lead.won_by || metadata.wonBy || metadata.won_by),
    lostAt: sanitizeString(lead.lostAt || lead.lost_at || metadata.lostAt || metadata.lost_at),
    lostBy: sanitizeString(lead.lostBy || lead.lost_by || metadata.lostBy || metadata.lost_by),
    lostReason: sanitizeString(lead.lostReason || lead.lost_reason || metadata.lostReason || metadata.lost_reason),
    lostNote: sanitizeString(lead.lostNote || lead.lost_note || metadata.lostNote || metadata.lost_note),
