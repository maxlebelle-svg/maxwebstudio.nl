...
export function renderProgressState(elements, progress) {
  ...
  elements.title.textContent = progress.productLabel || "Je websiteproject";
  elements.subtitle.textContent = progress.currentStepDescription
    ? `Nu bezig: ${lowercaseFirst(progress.currentStepDescription)}`
    : progress.currentPhase ? `Huidige fase: ${humanize(progress.currentPhase)}` : "Je project wordt bijgewerkt.";
  ...
  progressbar?.setAttribute("aria-label", `Voortgang van ${progress.productLabel || "je project"}`);
...
function renderLoading(elements) {
  ...
  elements.subtitle.textContent = "We halen je actuele projectstap veilig op.";
...
function renderUnavailable(elements) {
  ...
  elements.subtitle.textContent = "Je projectgegevens zijn tijdelijk niet als centrale voortgang beschikbaar. De rest van je portaal blijft gewoon werken.";
  ...
  elements.steps.replaceChildren(message("Neem gerust contact op als je wilt weten wat de actuele volgende stap is.", "neutral"));
...
function renderError(elements, state) {
  ...
  elements.subtitle.textContent = "Je overige klantportaalfuncties blijven beschikbaar. Probeer het later opnieuw of neem contact op.";
  elements.value.textContent = "Tijdelijk niet beschikbaar";
  elements.bar.style.width = "0%";
  elements.steps.replaceChildren(message("Je hoeft nu niets technisch te doen.", "neutral"));
  renderNextStep(elements, { available: false, contact: null });
  suppressConflictingOverview(elements);
}
...
function renderNextStep(elements, progress) {
  ...
  if (!progress.available) {
    ...
    elements.nextTitle.textContent = "We houden je project in beeld";
    elements.nextText.textContent = "Je actuele volgende stap is nu niet betrouwbaar beschikbaar. Neem contact op als je direct duidelijkheid wilt.";
  ...
  if (progress.customerActionRequired) {
    ...
    elements.nextTitle.textContent = safeAction ? action.label || "Jouw actie is nodig" : "We hebben iets van je nodig";
    elements.nextText.textContent = progress.blocked
      ? progress.blocker?.label || "Deze projectstap wacht op je aandacht."
  ...
  elements.nextTitle.textContent = progress.complete ? "Je traject is afgerond" : "Wij zijn aan zet";
  elements.nextText.textContent = progress.complete
    ? "De zichtbare stappen in dit traject zijn afgerond."
    : progress.nextStep?.label ? `Hierna: ${lowercaseFirst(progress.nextStep.label)}.` : "Zodra we iets van je nodig hebben, verschijnt dat hier.";
  elements.nextActions.append(contactLink(progress.contact));
}
...
function renderContact(elements, contact) {
  ...
  const safe = contact?.name ? contact : { name: "Team Max Webstudio", role: "Je vaste webstudioteam", email: "info@maxwebstudio.nl", phone: "085 130 5282", photoUrl: null, fallback: true };
  target.replaceChildren();
  ...
  const label = document.createElement("small"); label.textContent = "Jouw contact";
