...
function label(value) { return ({ invited:"Uitgenodigd",account_activated:"Account geactiveerd",in_progress:"Bezig",assessment_failed:"Toets onvoldoende",awaiting_documents:"Wacht op afronding",certified:"Gecertificeerd - activatie vereist",active:"Actief",paused:"Geschorst",revoked:"Ingetrokken" })[value] || value || "Onbekend"; }
...
function render() {
  ...
  for (const event of data.events || []) if (!events.has(event.partner_profile_id)) events.set(event.partner_profile_id, event);
  ...
  byId("partners").replaceChildren(...partners.map((partner) => {
    ...
    const latestEvent = events.get(partner.id);
    if (latestEvent) { const audit = document.createElement("span"); audit.textContent = `Laatste audit: ${latestEvent.event_type} · ${formatDate(latestEvent.occurred_at)}`; current.append(audit); }
    ...
    if (certificate?.status === "valid" && ["certified","paused"].includes(onboarding.status)) { const button = document.createElement("button"); button.className="access"; button.textContent=onboarding.status === "paused" ? "Heractiveren" : "Account activeren"; button.addEventListener("click", () => changeAccess(partner.id,"activate_partner")); cert.append(button); }
    if (onboarding.status === "active") { const button = document.createElement("button"); button.className="danger"; button.textContent="Schorsen"; button.addEventListener("click", () => changeAccess(partner.id,"suspend_partner")); cert.append(button); }
...
async function changeAccess(partnerProfileId, action) { const verb=action === "activate_partner" ? "activatie/heractivatie" : "schorsing"; const reason=prompt(`Geef een concrete reden voor ${verb} (minimaal 5 tekens):`); if(!reason)return; try{state.data=await request("",{action,partnerProfileId,reason,idempotencyKey:crypto.randomUUID()});render();}catch(error){alert(error.message);} }
