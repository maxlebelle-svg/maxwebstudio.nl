...
import { resolveAssetPackAssets } from "../../src/asset-pack-resolver.mjs";
...
function resolveFactoryAssetPack(blueprint, services, seed) {
  const branch = blueprint.dimensions.vertical.slug;
  const specialization = blueprint.dimensions.specialization?.id || null;
  const resolved = resolveAssetPackAssets({ branch, specialization }, { seed });
  const selections = (resolved.selections || []).map((selection) => ({
    assetSlot: selection.asset_slot,
  ...
  const serviceAssets = selections.find((selection) => selection.assetSlot === "service")?.assets || [];
  return {
    ...
    specialization,
    selections,
    serviceBindings: services.map((service, index) => ({
      branch,
      specialization,
      service: service.name,
      assetId: serviceAssets.length ? serviceAssets[index % serviceAssets.length].assetId : null,
      fallback: serviceAssets.length ? null : "existing_v1_v2_asset_resolution"
    }))
  };
}
...
export function resolveWebsiteContentV2(input = {}) {
  ...
  const assetPack = resolveFactoryAssetPack(blueprint, services, seed);
  ...
  const result = {
    ...
    websiteFactoryInput: {
      ...
      content: {
        ...
        layoutEngine: {
          version: "layout-engine/v2",
          family: blueprint.design_system.layout.hero,
          sequence: ["hero", "cta", `branch:${blueprint.dimensions.vertical.slug}`, "contact"]
        },
        serviceEngine: {
          version: "service-engine/v2",
          branch: blueprint.dimensions.vertical.slug,
          specialization: blueprint.dimensions.specialization?.id || null,
          services: services.map((service) => service.name)
        },
        assetPack,
        designEngine: {
          version: "design-engine/v2",
          branch: blueprint.dimensions.vertical.slug,
          style: blueprint.dimensions.visual_style.id,
          personality: blueprint.dimensions.brand_personality.id,
          tokens: blueprint.design_system
        },
        assets
      },
      seo: {
