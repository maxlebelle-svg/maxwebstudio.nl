...
insert into public.factory_projects(id,relationship_type,relationship_id,factory_type,blueprint_key,blueprint_version,name,status,created_by,updated_at) values
  ('40000000-0000-4000-8000-000000000005','customer','30000000-0000-4000-8000-000000000001','food','food-pickup-v1',1,'Generation gate','review','20000000-0000-4000-8000-000000000002','2026-07-30T12:00:00Z'),
  ('40000000-0000-4000-8000-000000000006','customer','30000000-0000-4000-8000-000000000001','food','food-pickup-v1',1,'Unbound override gate','review','20000000-0000-4000-8000-000000000002','2026-07-30T12:00:00Z'),
  ('40000000-0000-4000-8000-000000000007','customer','30000000-0000-4000-8000-000000000001','food','food-pickup-v1',1,'Dependent source gate','review','20000000-0000-4000-8000-000000000002','2026-07-30T12:00:00Z');

create function pg_temp.food_gate_batch(input_status text default 'passed') returns jsonb
language sql as $$
...
do $$ declare generation jsonb; generation_id uuid; result jsonb; begin
  ...
  perform public.factory_store_gate_checks_v1(
    '40000000-0000-4000-8000-000000000007','20000000-0000-4000-8000-000000000002','dependent-store-0001',generation_id,pg_temp.food_gate_batch()
  );
...
do $$ declare generation jsonb; generation_id uuid; result jsonb; current_updated_at timestamptz; begin
  ...
  generation_id := (generation->>'generationId')::uuid;
  perform public.factory_store_gate_checks_v1(
    '40000000-0000-4000-8000-000000000005','20000000-0000-4000-8000-000000000002','generation-store-0001',generation_id,pg_temp.food_gate_batch()
  );
  ...
  generation_id := (generation->>'generationId')::uuid;
  perform public.factory_store_gate_checks_v1(
    '40000000-0000-4000-8000-000000000005','20000000-0000-4000-8000-000000000002','generation-store-0002',generation_id,pg_temp.food_gate_batch()
  );
  result := public.factory_authorize_live_v1(
...
do $$ declare authorized_count integer; begin
  ...
  if exists (
    ...
    where created_at >= '2026-07-30T00:00:00Z' and event_type not in ('check_reported','check_expired','preflight_requested')
