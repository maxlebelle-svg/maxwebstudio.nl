...
test("the projection is scoped to the secured Demo Journey GET response", () => {
  ...
  assert.match(readSource, /projectBuildHistoryMetadata\(safeFactoryHistory\)/);
  assert.doesNotMatch(remainingSource, /projectBuildHistoryMetadata\(safeFactoryHistory\)/);
  ...
  assert.match(customerSource, /return jsonResponse\(200, \{ success: true, journey, previewVersion: sanitizeCustomerPreviewVersion\(activePreview\), events \}\)/);
