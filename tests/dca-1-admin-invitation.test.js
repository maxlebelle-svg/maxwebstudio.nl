...
test("startpagina toont minimale Nederlandse CX2-presentatie zonder interne identifiers", () => { const publicSource = `${startHtml}\n${startJs}`; for (const text of ["Welkom", "Demo gereed", "Nog 3 stappen tot oplevering", "Bekijk mijn website", "100% veilig"]) assert.ok(publicSource.includes(text)); for (const forbidden of ["telefoonnummer", "customer_id", "preview_token"]) assert.equal(publicSource.toLowerCase().includes(forbidden), false); assert.match(publicSource, /gecontroleerde e-mailadres/i); });
