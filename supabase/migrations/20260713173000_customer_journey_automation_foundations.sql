...
create table if not exists public.automation_outbox (
  ...
  constraint automation_outbox_idempotency_key_format check (idempotency_key ~ '^[A-Za-z0-9][A-Za-z0-9._:/-]{2,255}$'),
...
create table if not exists public.automation_executions (
  ...
  constraint automation_executions_idempotency_key_format check (idempotency_key ~ '^[A-Za-z0-9][A-Za-z0-9._:/-]{2,255}$')
);
...
begin
  if p_event_key is null or p_event_key !~ '^[A-Za-z0-9][A-Za-z0-9._:-]{2,191}$' then
    raise exception 'invalid_event_key' using errcode = '22023';
  end if;
  if p_event_type is null or p_event_type !~ '^[a-z][a-z0-9._-]{2,127}$' then
  ...
  if p_entity_id is null or length(p_entity_id) > 200 then
    raise exception 'invalid_entity_id' using errcode = '22023';
  end if;
  if p_environment is null or p_environment not in ('production', 'test', 'demo') then
  ...
  if jsonb_typeof(coalesce(p_payload, '{}'::jsonb)) <> 'object'
     or jsonb_typeof(coalesce(p_effect_payload, '{}'::jsonb)) <> 'object' then
    raise exception 'invalid_payload' using errcode = '22023';
  end if;
  if (p_outbox_idempotency_key is null) <> (p_effect_type is null) then
    raise exception 'incomplete_outbox_effect' using errcode = '22023';
  end if;
  if p_outbox_idempotency_key is not null
     and p_outbox_idempotency_key !~ '^[A-Za-z0-9][A-Za-z0-9._:/-]{2,255}$' then
    raise exception 'invalid_outbox_idempotency_key' using errcode = '22023';
  end if;
  if p_effect_type is not null and p_effect_type !~ '^[a-z][a-z0-9._-]{2,127}$' then
    raise exception 'invalid_effect_type' using errcode = '22023';
  end if;
...
begin
  ...
  with claimable as (
    ...
    where item.environment = 'test'
      and item.effect_type in ('email.journey_test', 'email.preview_ready')
      and (
...
-- Fail the transaction when the deployed security/storage contract is not exact.
do $$
declare
  table_name text;
  relation regclass;
  function_oid oid;
begin
  if not exists (select 1 from pg_proc where proname = 'gen_random_uuid' and pronargs = 0) then
    raise exception 'missing_gen_random_uuid';
  end if;
  ...
  foreach table_name in array array[
    ...
    if relation is null then
      raise exception 'missing_journey_table: %', table_name;
    end if;
    ...
    if has_table_privilege('anon', relation, 'SELECT')
      ...
      raise exception 'direct_runtime_grant_detected: %', table_name;
    end if;
    if not has_table_privilege('service_role', relation, 'SELECT')
      ...
      raise exception 'service_role_grant_missing: %', table_name;
    end if;
    if not exists (
      ...
      raise exception 'service_role_policy_missing: %', table_name;
    end if;
  end loop;

  foreach function_oid in array array[
    to_regprocedure('public.record_journey_event_and_enqueue(text,text,text,text,uuid,uuid,jsonb,text,timestamptz,text,text,jsonb,timestamptz)')::oid,
    to_regprocedure('public.claim_automation_outbox(text,integer,integer,text)')::oid
  ] loop
    if function_oid is null then
      raise exception 'journey_rpc_missing';
    end if;
    if not (select prosecdef from pg_proc where oid = function_oid) then
      raise exception 'journey_rpc_not_security_definer';
    end if;
    if not coalesce((select proconfig @> array['search_path=public, pg_temp'] from pg_proc where oid = function_oid), false) then
      raise exception 'journey_rpc_search_path_invalid';
    end if;
    if has_function_privilege('anon', function_oid, 'EXECUTE')
       or has_function_privilege('authenticated', function_oid, 'EXECUTE') then
      raise exception 'journey_rpc_runtime_execute_detected';
    end if;
    if not has_function_privilege('service_role', function_oid, 'EXECUTE') then
      raise exception 'journey_rpc_service_role_execute_missing';
    end if;
  end loop;

  if to_regclass('public.automation_outbox_dispatch_idx') is null
     or to_regclass('public.automation_outbox_stale_lease_idx') is null
     or to_regclass('public.automation_executions_provider_message_idx') is null then
    raise exception 'journey_dispatch_index_missing';
  end if;
end
$$;
