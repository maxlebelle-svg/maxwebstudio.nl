-- Phase 8: allow the bounded test worker to claim the preview-approved mail effect.
...
begin
  ...
  with claimable as (
    ...
    where item.environment = 'test'
      and item.effect_type in ('email.journey_test', 'email.preview_ready', 'email.feedback_received', 'email.preview_approved')
