-- Phase 7: allow the existing bounded test worker to claim the feedback mail effect.
-- Additive/idempotent: no table, column, data, policy or grant is removed.
begin;
...
begin
  ...
  with claimable as (
    ...
    where item.environment = 'test'
      and item.effect_type in ('email.journey_test', 'email.preview_ready', 'email.feedback_received')
      and (
...
do $$
declare
  function_oid oid := to_regprocedure('public.claim_automation_outbox(text,integer,integer,text)')::oid;
begin
  if function_oid is null then raise exception 'claim_function_missing'; end if;
  if not exists (select 1 from pg_proc where oid = function_oid and prosecdef and proconfig @> array['search_path=public, pg_temp']) then
    raise exception 'claim_function_security_contract_invalid';
  end if;
  if has_function_privilege('anon', function_oid, 'EXECUTE') or has_function_privilege('authenticated', function_oid, 'EXECUTE') then
    raise exception 'claim_function_public_execute_not_revoked';
  end if;
  if not has_function_privilege('service_role', function_oid, 'EXECUTE') then
    raise exception 'claim_function_service_role_execute_missing';
  end if;
end
$$;
