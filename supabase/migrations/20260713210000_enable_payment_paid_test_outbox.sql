-- Phase 9: allow the bounded test worker to claim the payment-paid mail effect.
-- Additive/idempotent: no table, column, policy, grant or data row is removed.
begin;
...
begin
  ...
  with claimable as (
    ...
    where item.environment = 'test'
      and item.effect_type in ('email.journey_test', 'email.preview_ready', 'email.feedback_received', 'email.preview_approved', 'email.payment_paid')
