-- P0 production reconciliation: canonical normalizers and transactional lead intake.
...
begin
  if current_user <> 'postgres' then
    raise exception using errcode = '55000', message = 'P0 production reconciliation must run as postgres.';
  end if;
  if to_regclass('public.p0_staging_smoke_nonces') is not null then
    raise exception using errcode = '55000', message = 'Staging-only smoke nonce objects are forbidden in production.';
  end if;
...
create function public.mws_sync_lead_compatibility_v1()
returns trigger
language plpgsql
set search_path to 'pg_catalog'
as $compatibility$
begin
  if tg_op = 'INSERT' then
    ...
    if new.source is not null and new.external_source is not null and new.source is distinct from new.external_source then
      raise exception using errcode='23514', constraint='leads_source_compatibility_conflict', message='lead compatibility conflict: source';
    end if;
    ...
    new.source := coalesce(new.source, new.external_source);
    new.external_source := coalesce(new.external_source, new.source);
  else
    ...
    if new.source is distinct from old.source and new.external_source is distinct from old.external_source and new.source is distinct from new.external_source then
      raise exception using errcode='23514', constraint='leads_source_compatibility_conflict', message='lead compatibility conflict: source';
    elsif new.source is distinct from old.source then new.external_source := new.source;
    elsif new.external_source is distinct from old.external_source then new.source := new.external_source;
    end if;
  end if;
  return new;
end
...
alter function public.mws_sync_lead_compatibility_v1() owner to postgres;
revoke all on function public.mws_sync_lead_compatibility_v1() from public, anon, authenticated, service_role;

create trigger sync_lead_compatibility_v1
before insert or update of company, company_name, name, contact_name, website_url, website, source, external_source
on public.leads for each row execute function public.mws_sync_lead_compatibility_v1();
